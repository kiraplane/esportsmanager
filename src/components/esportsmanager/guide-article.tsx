...
import type { Guide } from '@/data/esportsmanager/types';
...
export function GuideArticle({
  ...
  return (
    <div className="bg-[#061018] py-10 text-[#F4FAFF]">
      <JsonLd data={jsonLd} />
      <AdsterraSideRails />
      <Container className="px-4">
        <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_264px]">
          <article className="min-w-0 space-y-6">
            <header className="rounded-lg border border-[#1F3A4A] bg-[#0E1B27] p-5 md:p-7">
              <div className="flex flex-wrap gap-2">
                ...
                <Badge
                  variant="outline"
                  className="border-[#59D6FF]/60 bg-[#061018]/60 text-[#F4FAFF]"
                >
                ...
                {guide.tags.map((tag) => (
                  <Badge
                    key={tag}
                    variant="outline"
                    className="border-[#59D6FF]/40 bg-[#061018]/50 text-[#F4FAFF]"
                  >
                    {tag}
                  </Badge>
                ))}
            ...
            <div className="overflow-hidden rounded-lg border border-[#1F3A4A] bg-[#0E1B27]">
              <div className="relative aspect-[16/7] min-h-[180px] border-[#1F3A4A] border-b">
                <Image
              ...
              <div className="p-5 md:p-8">
                {guide.video ? (
                  <section className="mb-8 overflow-hidden rounded-lg border border-[#1F3A4A] bg-black">
                    <iframe
                      ...
                      allowFullScreen
                    />
                    <div className="bg-[#061018] px-4 py-3 text-sm leading-6 text-[#BED3E0]">
                      {ui.videoPrefix}{' '}
                      <a
                ...
                <div className="space-y-8">
                  ...
                  {guide.body.map((section) => (
                    ...
                    >
                      ...
                      <div className="mt-3 space-y-4 text-base leading-8 text-[#BED3E0]">
                        {section.paragraphs.map((paragraph, index) => (
                          <p key={`${section.heading}-${index}`}>{paragraph}</p>
                        ))}
                      </div>
                      {section.bullets ? (
                        <ul className="mt-4 grid gap-2 text-sm leading-7 text-[#BED3E0]">
                          {section.bullets.map((bullet, index) => (
                            <li
                              key={`${section.heading}-bullet-${index}`}
                              className="rounded-md border border-[#1F3A4A] bg-[#061018] px-3 py-2"
                            >
                              {bullet}
                            </li>
                          ))}
                ...
                <section className="mt-10 rounded-lg border border-[#1F3A4A] bg-[#061018] p-5">
                  <div className="flex gap-3">
                    <ShieldCheck className="mt-1 size-5 shrink-0 text-[#74F2A7]" />
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-7 text-[#BED3E0]">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 text-[#59D6FF] underline underline-offset-4"
                        >
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    {nextRoutes.map((route) => (
                      <Button
                        ...
                        asChild
                        variant="outline"
                        className="h-auto min-w-0 justify-between gap-3 whitespace-normal border-[#1F3A4A] bg-[#0E1B27] px-3 py-3 text-left text-[#F4FAFF] hover:bg-[#143047]"
                      >
